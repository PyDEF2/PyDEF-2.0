...
class ChemicalPotentialsCreationWindow(tk.Toplevel):
    ...
    def __init__(self, mainwindow, project, chemical_potentials_to_edit):
        ...
        all_phases = list(self.project.unboundcells.values())
        items = [cell.treetitle for cell in all_phases if not self.project.is_host(cell)]
        
        if self.chemical_potentials is not None:    
            items_on = [cell.treetitle for cell in list(self.chemical_potentials.non_synthesized.values())]
        else:
            items_on = []
        ...
        def validate(close=False):
            ...
            unboundcells = list(self.project.unboundcells.values())
            ...
            if not self.edit:
                ...
                ns = list(self.chemical_potentials.non_synthesized.values())
...
class PotentialsPlotParametersWindow(tk.Toplevel):
    ...
    def __init__(self, mainwindow, project, chemical_potentials):
        ...
        self.frame = tkinter.ttk.LabelFrame(self.main_frame, labelwidget=label0)
        self.label1 = tk.Label(self.frame, text='Plot Parameters Name', pady=10)
        namelist = [ppp.name for ppp in list(self.project.pp['ppp'].values())]
        self.pppname = tkinter.ttk.Combobox(self.frame, values=namelist, width=40, validate='focusout', validatecommand=self.ppp_select_create)
        ...
        self.xchoice = tkinter.ttk.Combobox(self.frame, values=self.atom_types, state="readonly")
        self.ychoice = tkinter.ttk.Combobox(self.frame, values=self.atom_types, state="readonly")
        self.cchoice = tkinter.ttk.Combobox(self.frame, values=self.atom_types, state="readonly")
        ...
        self.range_frame = tkinter.ttk.LabelFrame(self.main_frame, labelwidget=rangelabel)
        self.autoscale = tk.BooleanVar()
        self.autoscale.set(self.ppp.autoscale)
        
        tkinter.ttk.Checkbutton(self.range_frame, text='autoscale', variable=self.autoscale, onvalue=True,
        ...
        self.displayparam_frame = tkinter.ttk.LabelFrame(self.main_frame, labelwidget=displayparam)
        ...
        tkinter.ttk.Checkbutton(self.left_pane, text='display grid', variable=self.grid, onvalue=True,
                        offvalue=False, style='Bold.TCheckbutton', command=activate_range).grid(row=0,column=0, sticky='w')
        tkinter.ttk.Checkbutton(self.left_pane, text='hash stability domain', variable=self.hashed, onvalue=True,
                        offvalue=False, style='Bold.TCheckbutton', command=activate_range).grid(row=1,column=0, sticky='w')
        tkinter.ttk.Checkbutton(self.left_pane, text='highlight domain summits', variable=self.display_summits, onvalue=True,
        ...
        competing_phases = [cell.ID for cell in list(self.chemical_potentials.non_synthesized.values()) if cell.ID != self.chemical_potentials.synthesized.ID]
        ...
        self.equations_frame = tkinter.ttk.LabelFrame(self.main_frame, labelwidget=equations)
        ...
        def validate(close=False):
            ...
            if self.chemical_potentials.synth_population == 2:
                ...
                if self.chemical_potentials.synth_population > 3:
                    for species in list(self.remain_atoms.keys()):
    ...
    def ppp_select_create(self):
        ...
        if self.pppname.get() not in [ppp.name for ppp in list(self.project.pp['ppp'].values())]:
    ...
    def open_color_picker(self):
        self.colors = {}
        self.color_picker = tu.ColoursChoiceWindow(self, [cell.rname for cell in list(self.chemical_potentials.non_synthesized.values())], self.ppp.colors, self.colors)
