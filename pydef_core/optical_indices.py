...
class OpticalIndices(object):
    ...
    def plot_tensor(self, tensorid, pp=None, ax=None):
        ...
        if tensorid in self.quantities:
            ...
            data_dict = dict(list(zip(labels, data)))
            length = len(labels)
            
            if len(split) == 2:
                key_init_list = [[k, '$' + data_label + '_{' + i + ' ' + k + '}$'] for k in list(data_dict.keys())]
            else:
                key_init_list = [[k, '$' + data_label + '_{' + k + '}$'] for k in list(data_dict.keys())]
            ...
            if opp.trace_only:
                ...
                if opp.h_shift != 0:
                    ...
                    if tensorid.find('epsilon') > -1:
                        ...
                        reflect = ((n-1)**2+k**2)/((n+1)**2+k**2)
                        abso = 4*np.pi*self.energy*k
                        loss = ep2/(ep1**2 + ep2**2)
                        sigma1 = self.energy*ep1/(4*np.pi)
                        sigma2 = self.energy*ep2/(4*np.pi)
                        val = {'n':n, 'k':k, 'R': reflect, 'abs': abso, 'L': loss, 'sigma1': sigma1, 'sigma2': sigma2}
                ...
                for key1 in list(key_to_label.keys()):
                    if key1 != 'XX':
                        try:
                            if bf.is_zero(data_dict[key1]):
                                ...
                                for key2 in list(data_dict.keys()):
                                    try:
                ...
                if opp.h_shift != 0:
                    if tensorid.find('epsilon') > -1:
                        ...
                        def abso(key):
                            return 4*np.pi*self.energy*k(key)
                        def loss(key):
                            return -eps2[key][0]**2/(eps1[key][0]**2+eps2[key][0]**2)
                        def sigma1(key):
                            return self.energy*eps1[key][0]/(4*np.pi)
                        def sigma2(key):
                            return self.energy*eps2[key][0]/(4*np.pi)
            ...
            pf.set_ax_parameters(ax, title=opp.title, xlabel=opp.x_label, ylabel=opp.y_label, xlim=[opp.xmin, opp.xmax], ylim=[opp.ymin, opp.ymax], legend=opp.display_legends, grid=opp.grid,
                             ...
                             ytick_labels=opp.yticklabels_var, title_fontsize=opp.title_fontsize)
...
def get_nkr(eps1, eps2, energy):
    ...
    abso = 4*np.pi*energy*k
    loss = eps2/(eps1**2 + eps2**2)
    sigma1 = energy*eps1/(4*np.pi)
    sigma2 = energy*eps2/(4*np.pi)

    return n, k, r, abso, loss, sigma1, sigma2
...
class OpticalPlotParameters(pf.PlotParameters):
    ...
    def __init__(self, optical_indices):
        ...
        self.data_id = ['epsilon 1', 'epsilon 2', 'n', 'k', 'R', 'abs', 'L', 'sigma1', 'sigma2']
        ...
        #print(optical_indices.data_dict.keys())
        ...
        #colormaps = ['Reds', 'Blues', 'Greens', 'Oranges', 'Purples', 'Greys','cyan']
        colormaps = ['Greys', 'Purples', 'Blues', 'Greens', 'Oranges', 'Reds','YlOrBr', 'YlOrRd', 'OrRd']
        p = -1
        for q in optical_indices.quantities:
            p += 1
            customcmap = plt.get_cmap(colormaps[p-1])
            self.colors[q] = dict(list(zip(optical_indices.components, [rgb2hex(customcmap(120+18*i)) for i in range(0, 7)]))) 
